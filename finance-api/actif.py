...
class actif:
  ...
  def get_donnees_seance(self):
    ...
    u=['Capitalisation', 'Cours', 'Cours de cloture veille','Devise de cotation','Nombre de titres','Ouverture','Plus bas','Plus haut',\
  ...
  def get_meilleur_limite(self):
    ...
    # Meilleur Limite
    u=['Prix achat','Quantite_achat','Prix de vente','Quantite_vente']
    v=[''] * 4   
    for i in range(4):
  ...
  def get_info_societe(self):
    ...
    u=['Commissaire_aux_comptes','Date_de_constitution','Date_introduction','Durée_Exercice_Social',\
  ...
  def recap_volume(self):
    ...
    print(load.getIndexRecap()['Volume Global'])
